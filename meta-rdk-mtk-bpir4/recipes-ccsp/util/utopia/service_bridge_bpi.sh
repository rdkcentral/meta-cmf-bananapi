...
#Create a virtual net0 management interface and connect it to the bride
...
virtual_interface()
{
   echo "Inside virtual_interface"
    ...
    if [ "$1" = "enable" ] ; then
        ...
        echo 1 > /proc/sys/net/ipv6/conf/lnet0/disable_ipv6

        echo "ifconfig $CMDIAG_IF hw ether `cat /sys/class/net/net0/address`"
...
virtual_interface_ebtables_rules ()
{
    ...
    if [ -z "$EROUTER_MAC" ]; then
        echo "No valid MAC address found for lan0, or erouter0."
    fi
...
##Filter table
           #--------------------------------------------------------------------------------------
           #####Forward rules for virtual interface(Dont allow net0 to send traffic to erouter0)
           #--------------------------------------------------------------------------------------
...
##NAT TABLE
         #--------------------------------------------------------------------------------------
         ####Redirect traffic destined to net0 IP to net0 MAC address from brlan0(Prerouting rules)
         #--------------------------------------------------------------------------------------
...
add_to_group()
{
  ...
  ifconfig "$lan_ethernet_ifnamei" up
...
service_start ()
{
   ...
   if [ "started" != "$STATUS" ] ; then
         ...
         virtual_interface enable #create net0 interface and write ebtable rules
