...
do_kernel_metadata() {
	...
	if [ -f "${STAGING_KERNEL_DIR}/scripts/util/configme" ]; then
		if [ -z "${EXTERNAL_KERNEL_TOOLS}" ]; then
			PATH=${STAGING_KERNEL_DIR}/scripts/util:${PATH}
		fi
	fi
	...
	if [ -n "${KBUILD_DEFCONFIG}" ]; then
		if [ -f "${STAGING_KERNEL_DIR}/arch/${ARCH}/configs/${KBUILD_DEFCONFIG}" ]; then
			if [ -f "${UNPACKDIR}/defconfig" ]; then
				...
				cmp "${UNPACKDIR}/defconfig" "${STAGING_KERNEL_DIR}/arch/${ARCH}/configs/${KBUILD_DEFCONFIG}"
				if [ $? -ne 0 ]; then
					bbdebug 1 "detected SRC_URI or patched defconfig in UNPACKDIR. ${KBUILD_DEFCONFIG} copied over it"
				fi
			fi
			cp -f ${STAGING_KERNEL_DIR}/arch/${ARCH}/configs/${KBUILD_DEFCONFIG} ${UNPACKDIR}/defconfig
			in_tree_defconfig="${UNPACKDIR}/defconfig"
		else
			bbfatal "A KBUILD_DEFCONFIG '${KBUILD_DEFCONFIG}' was specified, but not present in the source tree (${STAGING_KERNEL_DIR}/arch/${ARCH}/configs/)"
		fi
	fi
	...
	includes="$includes -I${STAGING_KERNEL_DIR}/arch/${ARCH}/configs -I${STAGING_KERNEL_DIR}/kernel/configs"
	...
	if [ "$mode" = "config" ]; then
		...
		if [ -n "${elements}" ]; then
			echo "${bsp_definition}" > ${STAGING_KERNEL_DIR}/${meta_dir}/bsp_definition
			echo "${KMETA_CONFIG_FEATURES}" | grep -q "prefer-modules"
			if [ $? -eq 0 ]; then
				scc_defines="-DMODULE_OR_Y=m"
			fi
			scc --force $scc_defines -o ${STAGING_KERNEL_DIR}/${meta_dir}:cfg,merge,meta ${includes} $sccs_defconfig $bsp_definition $sccs $patches $KERNEL_FEATURES_FINAL
			if [ $? -ne 0 ]; then
	...
	if [ "$mode" = "patch" ]; then
		...
		if [ -n "${elements}" ]; then
			scc --force -o ${STAGING_KERNEL_DIR}/${meta_dir}:patch --cmds patch ${includes} ${sccs} ${patches} $KERNEL_FEATURES_FINAL
			if [ $? -ne 0 ]; then
				bbfatal_log "Could not generate configuration queue for ${KMACHINE}."
			fi
		fi
...
do_patch() {
	...
	if [ -f "${meta_dir}/series" ]; then
		...
		if [ $? -ne 0 ]; then
			...
			bbfatal_log "Patch failures can be resolved in the linux source directory ${STAGING_KERNEL_DIR})"
		fi
	fi
...
do_kernel_checkout() {
	...
	source_dir=`echo ${STAGING_KERNEL_DIR} | sed 's%/$%%'`
	...
	if [ -d "${source_unpackdir}" ]; then
		...
		if [ "${source_dir}" != "${source_unpackdir}" ]; then
			if [ -d "${source_unpackdir}/.git" ]; then
				...
				mv ${source_unpackdir} ${STAGING_KERNEL_DIR}
			else
				# create source for bare cloned git repository
				git clone ${source_unpackdir} ${STAGING_KERNEL_DIR}
		...
		# if ${STAGING_KERNEL_DIR} hasn't been set to the proper subdirectory a default of "linux" is 
...
do_kernel_checkout[dirs] = "${STAGING_KERNEL_DIR} ${UNPACKDIR}"
...
# ${STAGING_KERNEL_DIR} doesn't exist for us at unpack
do_qa_unpack() {
    return
}
...
do_kernel_configme[dirs] += "${STAGING_KERNEL_DIR} ${B}"
do_kernel_configme() {
	...
	if [ $? -ne 0 -o ! -f ${B}/.config ]; then
		...
		if [ ${KCONF_AUDIT_LEVEL} -gt 1 ]; then
			...
			bbfatal_log "Details can be found at: ${STAGING_KERNEL_DIR}/${meta_dir}/cfg/merge_config_build.log"
		fi
	fi
...
python do_kernel_configcheck() {
    ...
    if not kmeta or not os.path.exists('{}/{}'.format(s,kmeta)):
        kmeta = subprocess.check_output(['kgit', '--meta'], cwd=d.getVar('STAGING_KERNEL_DIR')).decode('utf-8').rstrip()
...
do_validate_branches() {
	...
	if [ "${machine_srcrev}" = "AUTOINC" ]; then
		...
		ver=$(grep "^VERSION =" ${STAGING_KERNEL_DIR}/Makefile | sed s/.*=\ *//)
		patchlevel=$(grep "^PATCHLEVEL =" ${STAGING_KERNEL_DIR}/Makefile | sed s/.*=\ *//)
		sublevel=$(grep "^SUBLEVEL =" ${STAGING_KERNEL_DIR}/Makefile | sed s/.*=\ *//)
		kver="$ver.$patchlevel"
