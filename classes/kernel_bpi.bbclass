...
# Old style kernels may set ${STAGING_KERNEL_DIR} = ${WORKDIR}/git for example
# We need to move these over to STAGING_KERNEL_DIR. We can't just
# create the symlink in advance as the git fetcher can't cope with
# the symlink.
do_unpack[cleandirs] += " ${STAGING_KERNEL_DIR} ${STAGING_KERNEL_DIR} ${B} ${STAGING_KERNEL_BUILDDIR}"
do_clean[cleandirs] += " ${STAGING_KERNEL_DIR} ${STAGING_KERNEL_DIR} ${B} ${STAGING_KERNEL_BUILDDIR}"
...
kernel_do_compile() {
	...
	if [ "${KERNEL_DEBUG_TIMESTAMPS}" != "1" ]; then
		...
		if [ "${SOURCE_DATE_EPOCH}" = "" -o "${SOURCE_DATE_EPOCH}" = "0" ]; then
			...
			SOURCE_DATE_EPOCH=`git --git-dir="${STAGING_KERNEL_DIR}/.git" log -1 --pretty=%ct 2>/dev/null || echo "${REPRODUCIBLE_TIMESTAMP_ROOTFS}"`
		fi
...
kernel_do_install() {
	...
	#
	bbnote "no modules to install sandhya $KERNEL_IMAGEDEST"
	unset CFLAGS CPPFLAGS CXXFLAGS LDFLAGS MACHINE
...
# Must be ran no earlier than after do_kernel_checkout or else Makefile won't be in ${STAGING_KERNEL_DIR}/Makefile
do_kernel_version_sanity_check() {
	...
	VERSION=$(grep "^VERSION =" ${STAGING_KERNEL_DIR}/Makefile | sed s/.*=\ *//)
	PATCHLEVEL=$(grep "^PATCHLEVEL =" ${STAGING_KERNEL_DIR}/Makefile | sed s/.*=\ *//)
	SUBLEVEL=$(grep "^SUBLEVEL =" ${STAGING_KERNEL_DIR}/Makefile | sed s/.*=\ *//)
	EXTRAVERSION=$(grep "^EXTRAVERSION =" ${STAGING_KERNEL_DIR}/Makefile | sed s/.*=\ *//)
...
KERNEL_CONFIG_COMMAND ?= "oe_runmake_call -C ${STAGING_KERNEL_DIR} O=${B} olddefconfig || oe_runmake -C ${STAGING_KERNEL_DIR} O=${B} oldnoconfig"
...
kernel_do_configure() {
	...
	if [ ! -e ${B}/.scmversion -a ! -e ${STAGING_KERNEL_DIR}/.scmversion ]; then
		echo ${KERNEL_LOCALVERSION} > ${B}/.scmversion
		echo ${KERNEL_LOCALVERSION} > ${STAGING_KERNEL_DIR}/.scmversion
	fi

	if [ "${STAGING_KERNEL_DIR}" != "${B}" ] && [ -f "${STAGING_KERNEL_DIR}/.config" ] && [ ! -f "${B}/.config" ]; then
		mv "${STAGING_KERNEL_DIR}/.config" "${B}/.config"
	fi
